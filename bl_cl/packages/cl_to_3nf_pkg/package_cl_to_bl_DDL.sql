CREATE OR REPLACE PACKAGE cl_to_bl_pkg
   AUTHID DEFINER
IS
...
CREATE OR REPLACE PACKAGE BODY cl_to_bl_pkg
IS
    PROCEDURE merge_coupons
    IS    
    BEGIN
        MERGE INTO bl_3nf.dbo.ce_coupons cc
        USING (SELECT * FROM cl_coupons)bc ON (bc.coupon_id = cc.coupon_surrogate_id)
        ...
        VALUES
        (bl_3nf.dbo.sequence_common_1.NEXTVAL, bc.coupon_id, bc.coupon_desc, bc.issued_quantity);
        COMMIT;
...
IS
BEGIN
 MERGE INTO bl_3nf.dbo.ce_employees ce
    USING (SELECT * FROM cl_employees)cl ON (ce.employee_surrogate_id = cl.employee_id)
    WHEN MATCHED THEN 
        UPDATE SET
 ...
 ce.employee_surname, ce.age, ce.email, ce.phone )
    VALUES
    (bl_3nf.dbo.employees.NEXTVAL, cl.employee_id, cl.first_name,
...
IS
BEGIN
 MERGE INTO bl_3nf.dbo.ce_payment_types ce
    USING (SELECT * FROM cl_payment_types )cl ON (ce.payment_type_surrogate_id = cl.payment_type_id)
    WHEN MATCHED THEN 
        UPDATE SET
    ...
    VALUES
    (bl_3nf.dbo.sequence_common_1.NEXTVAL, cl.payment_type_id, cl.payment_type_desc);
...
PROCEDURE insert_stores
IS
CURSOR cur1 IS SELECT country FROM cl_stores
        GROUP BY country;
country_curs cl_stores.country%type;
BEGIN
    OPEN cur1;
    LOOP
    FETCH cur1 INTO country_curs;
    INSERT INTO bl_3nf.dbo.ce_countries(country_id, country_name)
    SELECT bl_3nf.dbo.sequence_common_2.NEXTVAL, country_curs FROM dual;
    ...
    INSERT INTO bl_3nf.dbo.ce_states(state_id, state_name, country_id)
    SELECT bl_3nf.dbo.sequence_common_2.NEXTVAL, state_province, cc.country_id FROM
        (SELECT  state_province, country FROM cl_stores
        GROUP BY state_province, country)a
    JOIN bl_3nf.dbo.ce_countries cc ON cc.country_name = a.country;
    log_event('Inserted ' || sql%Rowcount || ' rows into table ce_states');
    
    INSERT INTO bl_3nf.dbo.ce_cities(city_id, city_name, state_id)
    SELECT bl_3nf.dbo.sequence_common_2.NEXTVAL, city, cs.state_id FROM
        (SELECT  city, state_province FROM cl_stores
        GROUP BY city, state_province)a
    JOIN bl_3nf.dbo.ce_states cs ON cs.state_name = a.state_province;
    log_event('Inserted ' || sql%Rowcount || ' rows into table ce_cities');
    
    INSERT INTO bl_3nf.dbo.ce_postcodes( postcode_id, postcode )
    SELECT bl_3nf.dbo.sequence_common_2.NEXTVAL, postcode FROM
    ...
    SELECT bl_3nf.dbo.stores.NEXTVAL, ss.store_number, ss.store_name, ss.street_address, ss.phone_number,
    ci.city_id,cp.postcode_id FROM cl_stores ss
    JOIN bl_3nf.dbo.ce_postcodes cp ON ss.postcode = cp.postcode
    JOIN bl_3nf.dbo.ce_cities ci ON ci.city_name = ss.city
    JOIN bl_3nf.dbo.ce_states cs ON cs.state_id = ci.state_id
    JOIN bl_3nf.dbo.ce_countries ccount ON ccount.country_id = cs.country_id
