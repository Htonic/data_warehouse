CREATE OR ALTER  PROCEDURE bl_qa.dim_employees_completeness
AS
...
BEGIN
    ...
    WITH nf as 
		(SELECT employee_id,employee_natural_id,  employee_name,
    employee_surname, age, email, phone, update_dt
    FROM bl_3nf.dbo.ce_employees),

	dm as (SELECT employee_id,employee_natural_id,  employee_name,
    employee_surname, age, email, phone, update_dt FROM bl_dm.dbo.dim_employees)
	...
	SELECT 'Check completeness between bl_3nd and bl_dm for employees' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check completeness between bl_3nd and bl_dm for employees finished with status_id: ', @status_id);
