CREATE OR ALTER  PROCEDURE bl_qa.dim_payment_types_types_check
AS
DECLARE @failed_count INT;
DECLARE @failed_perc DECIMAL(18, 2);
DECLARE @status_id  INT;
DECLARE @table_name NVARCHAR(20) = 'dim_payment_types';
...
BEGIN
	...
	('payment_type_desc NOT NULL nvarchar(20)'),
	('payment_type_surrogate_id NOT NULL int'),
	('payment_type_id NOT NULL int'),
	('payment_type_natural_id NOT NULL int'),
	...
	SELECT 'Data types check for dim_payment_types' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check data types for dim_payment_types finished with status_id: ', @status_id);
