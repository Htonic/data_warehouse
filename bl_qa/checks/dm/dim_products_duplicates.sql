CREATE OR ALTER  PROCEDURE bl_qa.dim_products_duplicates
AS
...
BEGIN
	...
	WITH duplicated_records as (
		SELECT COUNT(*) as cnt_dm FROM bl_dm.dbo.dim_products where is_active = '1' group by product_natural_id having count(*) > 1)
	SELECT @failed_count = COUNT(*) FROM duplicated_records;

	IF(@failed_count <> 0)
	  BEGIN
		WITH  dm as (SELECT COUNT(*) as cnt_dm FROM bl_dm.dbo.dim_products  where is_active = '1')
	...
	SELECT 'Check duplicates for dim_products' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check duplicates for dim_products finished with status_id: ', @status_id);
