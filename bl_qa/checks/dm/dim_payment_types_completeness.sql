CREATE OR ALTER  PROCEDURE bl_qa.dim_payment_types_completeness
AS
...
BEGIN
    ...
    WITH nf as 
		(SELECT  payment_type_id,payment_type_natural_id, payment_type_desc, update_dt
    FROM bl_3nf.dbo.ce_payment_types),

	dm as (SELECT payment_type_id,payment_type_natural_id, payment_type_desc, update_dt FROM bl_dm.dbo.dim_payment_types)
	...
	IF(@failed_count <> 0)
	  BEGIN
		WITH dm as (SELECT COUNT(*) as cnt_dm FROM bl_dm.dbo.dim_payment_types)
	...
	SELECT 'Check completeness between bl_3nd and bl_dm for payment types' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check completeness between bl_3nd and bl_dm for payment types finished with status_id: ', @status_id);
