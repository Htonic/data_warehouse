CREATE OR ALTER  PROCEDURE bl_qa.dim_coupons_completeness
AS
...
BEGIN
    ...
    WITH nf as 
		(SELECT  coupon_id,coupon_natural_id, coupon_desc, issued_quantity, update_dt
    FROM bl_3nf.dbo.ce_coupons),

	dm as (SELECT coupon_id,coupon_natural_id, coupon_desc, issued_quantity, update_dt FROM bl_dm.dbo.dim_coupons)
	...
	IF(@failed_count <> 0)
	  BEGIN
		WITH dm as (SELECT COUNT(*) as cnt_dm FROM bl_dm.dbo.dim_coupons)
	...
	SELECT 'Check completeness between bl_3nd and bl_dm for coupons' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check completeness between bl_3nd and bl_dm for coupons finished with status_id: ', @status_id);
