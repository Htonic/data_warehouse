CREATE OR ALTER  PROCEDURE bl_qa.dim_product_count
AS
...
BEGIN
    ...
    WITH nf as (SELECT COUNT(*) as cnt_nf FROM bl_3nf.dbo.ce_products where is_active = '1'),
	dm as (SELECT COUNT(*) as cnt_dm FROM bl_dm.dbo.dim_products where is_active = '1')
	...
	SELECT 'Check counts between bl_3nd and bl_dm for products' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check counts between bl_3nd and bl_dm for products finished with status_id: ', @status_id);
