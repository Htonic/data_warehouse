CREATE OR ALTER  PROCEDURE bl_qa.cl_payment_types_completeness
AS
...
BEGIN
    ...
    WITH src as 
		(SELECT CAST(payment_type_id AS INT) payment_type_id, trim(payment_type_desc) payment_type_desc
			FROM sa_src.dbo.src_payment_types), 

	cl as (SELECT payment_type_id, payment_type_desc FROM bl_cl.dbo.cl_payment_types)
	...
	IF(@failed_count <> 0)
	  BEGIN
		WITH cl as (SELECT COUNT(*) as cnt_cl FROM bl_cl.dbo.cl_payment_types)
	...
	SELECT 'Check completeness between sa_src and bl_cl for payment types' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check completeness between sa_src and bl_cl for payment types finished with status_id: ', @status_id);
