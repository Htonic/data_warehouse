CREATE OR ALTER  PROCEDURE bl_qa.cl_coupons_completeness
AS
...
BEGIN
    ...
    WITH src as 
		(SELECT CAST(TRIM(coupon_id) as INT) coupon_id, TRIM(coupon_desc) coupon_desc,
    CAST(issued_quantity as INT) issued_quantity FROM sa_src.dbo.src_coupons), 

	cl as (SELECT coupon_id,coupon_desc,issued_quantity FROM bl_cl.dbo.cl_coupons)
	...
	IF(@failed_count <> 0)
	  BEGIN
		WITH cl as (SELECT COUNT(*) as cnt_cl FROM bl_cl.dbo.cl_coupons)
	...
	SELECT 'Check completeness between sa_src and bl_cl for coupons' as check_name,
	@failed_perc,
	...
	SET @event_message = CONCAT('Check completeness between sa_src and bl_cl for coupons finished with status_id: ', @status_id);
